...
def generate_connection(config):
    ...
    visualize = config["connection_visualize"]
    mask_dir = config["mask"]
    connection_dir = config["connection"]

    # h = w = size
    mask_dir = os.path.join(root_dir, mask_dir)
    connection_dir = os.path.join(root_dir, connection_dir)
    if not os.path.exists(connection_dir):
        os.mkdir(connection_dir)
    if visualize:
        connection_visualize_dir = connection_dir + "_visualize"
        if not os.path.exists(connection_visualize_dir):
            os.mkdir(connection_visualize_dir)
    for mask in tqdm(os.listdir(mask_dir), desc="Generating connection ..."):
        ...
        if visualize:
            num_components = len(np.unique(labeled_mask))
            assert num_components > 0, f"cannot visualize file: {mask_name}.npy"
            colormap = create_colormap(num_components)
            colored_image = apply_colormap(labeled_mask, colormap)
            cv2.imwrite(os.path.join(connection_visualize_dir, mask_name + ".png"), colored_image)
...
def generate_weight(config):
    root_dir = config["root_path"]
    size = config["image_size"]
    visualize = config["weight_visualize"]
    weight_dir = config["weight"]
    connection_dir = config["connection"]
    h = w = size
    labeled_mask_dir = os.path.join(root_dir, connection_dir)
    weight_graph_dir = os.path.join(root_dir, weight_dir)
    if not os.path.exists(weight_graph_dir):
        os.mkdir(weight_graph_dir)
    labeled_mask_list = os.listdir(labeled_mask_dir)
    if visualize:
        weight_visualize_dir = weight_graph_dir + "_visualize"
